...
function evidenceText(d){return({NO_FILTERING_SIGNAL:"Сигналов фильтрации нет",NO_OUTAGE_SIGNAL:"Сигналов массового сбоя нет",OPERATIONAL:"Официальный статус: работает",PROVIDER_INCIDENT:"Официальная страница сообщает о проблеме"})[d]||diagText(d)}
...
async function openDetail(id){
  try{var d=await api("/api/resources/"+id),r=d.resource,local=r.your_network||{},global=r.global||{},timeline=d.timeline||[];S.detailId=id;q("#detailTitle").textContent=r.name;q("#detailBody").innerHTML='<div class="detail-top"><div class="detail-kv"><span>Вывод</span><b>'+diagText(r.diagnosis)+'</b></div><div class="detail-kv"><span>Глобально</span><b>'+stText(global.status)+'</b></div><div class="detail-kv"><span>Ваша сеть</span><b>'+(local.status?stText(local.status):"Недоступно")+'</b></div><div class="detail-kv"><span>Отклик</span><b>'+num((local.status?local:global).response_time_ms," мс")+'</b></div></div><div class="detail-section"><h3>'+esc(r.target)+'</h3><div class="detail-kv"><span>Пояснение</span><b>'+esc(r.diagnosis_text||"—")+'</b></div></div><div class="detail-section"><h3>Последние проверки</h3><div class="check-list">'+(d.checks.length?d.checks.map(function(c){return '<div class="check-row"><div>'+fmt(c.checked_at)+'</div><div>'+esc(c.probe_scope||"")+' · '+stText(c.status)+'</div><div>'+esc(c.message||"")+'</div><div>'+c.response_time_ms+' мс</div></div>'}).join(""):empty("Проверок нет",""))+'</div></div><div class="detail-section"><h3>История событий и внешних данных · до 12 месяцев</h3><div class="evidence-timeline">'+(timeline.length?timeline.slice(0,20).map(function(item){var summary=item.summary||{},title=item.type==="incident_opened"?"Инцидент открыт":item.type==="incident_closed"?"Инцидент закрыт":item.type==="status_change"?"Изменение состояния":item.type==="evidence"?String(item.source||"").toUpperCase()+" · "+evidenceText(item.classification):String(item.source||"").toUpperCase()+" · "+diagText(item.classification);var note=summary.description||summary.message||(summary.previous_status?summary.previous_status+" → "+(item.status||""):"");return '<article class="evidence-timeline-row"><time>'+fmt(item.time)+'</time><b>'+esc(title)+'</b><span>'+esc(note||item.status||"")+'</span>'+(item.confidence?'<small>Уверенность: '+esc(item.confidence)+'</small>':'')+'</article>'}).join(""):empty("Событий пока нет","После проверок и диагностики здесь появится история."))+'</div></div>';openDialog(q("#detailDialog"),"#detailCheck")}catch(e){toast(e.message)}
}
