...
var S={
  ...
  streamMinutes:60,timelineFilters:{GLOBAL:"ALL",RUSSIA:"ALL"},detailId:null,diagId:null,
...
var DASHBOARD_PANEL_ORDER=["resources_kpi","domestic_kpi","latency_kpi","personal_kpi","incidents_kpi","events","map","resources_table","fault_domain"];

function defaultDashboardPreferences(){
  return{
    panels:{
      resources_kpi:true,domestic_kpi:true,latency_kpi:true,personal_kpi:true,incidents_kpi:true,
...
function stText(s){return({OK:"Доступен",HTTP_REJECTED:"Доступен · probe отклонён",DNS_ERROR:"Недоступен · DNS",TCP_ERROR:"Недоступен · TCP",TLS_ERROR:"Недоступен · TLS",HTTP_ERROR:"Недоступен · HTTP",TIMEOUT:"Недоступен · таймаут",BLOCKED_TARGET:"Недоступен · блокировка",UNKNOWN:"Не определено · нет результата",UNKNOWN_ERROR:"Ошибка проверки"})[s]||"Результат проверки неизвестен"}
...
function diagText(d){return({AVAILABLE:"Доступен",LIKELY_RESTRICTION:"Вероятное ограничение",LIKELY_OUTAGE:"Вероятное падение",EXTERNAL_PATH_ISSUE:"Проблема пути VPS",DOMESTIC_UNKNOWN:"Результат проверки РФ неизвестен",INSUFFICIENT_DATA:"Недостаточно измерений"})[d]||"Результат проверки неизвестен"}
...
function renderOverview(){
  ...
  var ruLatest=resources.map(function(r){var series=(S.realtimeDom&&S.realtimeDom.resources||[]).find(function(item){return Number(item.id)===Number(r.id)});return latestPoint(series||{})});
  var ruFresh=ruLatest.map(function(point){return point&&Date.now()/1000-Number(point.timestamp)<=1800?timelineState(point):"unknown"});
  var ruAvailable=ruFresh.filter(function(state){return state==="up"}).length;
  var ruUnknown=ruFresh.filter(function(state){return state==="unknown"}).length;
  ...
  q("#kpiRussiaResources").textContent=ruAvailable;
  q("#kpiRussiaResourcesDelta").textContent="из "+resources.length;
  q("#kpiRussiaResourcesHint").textContent=!resources.length?"добавьте ресурсы для проверки РФ":ruUnknown?ruUnknown+" без свежего результата РФ":"по свежим измерениям российского контура";
  q("#kpiRussiaResourcesHint").title=ruUnknown+" ресурсов не имеют подтверждённого свежего результата и не включены в число доступных";
  q("#kpiRussiaMini").innerHTML=ruFresh.map(function(state){return '<i class="'+state+'" title="'+(state==="up"?"Доступен":state==="down"?"Недоступен":"Нет свежего результата")+'"></i>'}).join("");
  q("#kpiRussiaMini").setAttribute("aria-label",ruAvailable+" доступно, "+(ruFresh.length-ruAvailable-ruUnknown)+" недоступно, "+ruUnknown+" без подтверждённого результата");
...
function timelineCurrentLabel(point,state,isStale){if(state==="up")return"Доступен";if(state==="down")return"Недоступен · "+stText(point&&point.status).replace(/^Недоступен · /,"");if(!point)return"Не проверялся за период";return isStale?"Измерение устарело":"Ошибка проверки · результат неизвестен"}
function timelineReason(point,state,isStale,staleAfter){
  ...
  return point.status?"Проверка завершилась без подтверждения доступности или отказа ("+stText(point.status)+"). В старой записи причина сбоя не сохранена; эта проверка повторится автоматически.":"Проверка завершилась без однозначного результата.";
}
...
function drawResourceTimeline(containerId,statusId,source,scope){
  ...
  var allRows=scopeRows(source),now=Date.now()/1000,staleAfter=scope==="RUSSIA"?1800:120,filter=S.timelineFilters[scope]||"ALL";
  var rows=allRows.filter(function(row){var point=latestPoint(row),state=point&&now-Number(point.timestamp)<=staleAfter?timelineState(point):"unknown";return filter==="ALL"||filter===state});
  var start=Number(source&&source.from||now-S.streamMinutes*60),end=Number(source&&source.to||now),span=Math.max(1,end-start),resources=S.dashboard&&S.dashboard.resources||[];
  qa('.timeline-filter[data-scope="'+scope+'"] [data-timeline-filter]').forEach(function(button){var active=button.dataset.timelineFilter===filter;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active))});
  if(status){var visibleChecks=rows.reduce(function(n,r){return n+(r.points||[]).length},0);status.textContent=filter==="ALL"?(visibleChecks?"наблюдений: "+visibleChecks:"нет данных"):"ресурсов: "+rows.length+" из "+allRows.length;status.className="stream-scope-badge "+(visibleChecks?"ok":"neutral")}
  if(!allRows.length){el.innerHTML=empty("Нет ресурсов","Добавьте ресурсы в мониторинг.");return}
  if(!rows.length){el.innerHTML=empty("Нет ресурсов в этом состоянии",filter==="UP"?"Сейчас нет доступных ресурсов в этом контуре.":filter==="DOWN"?"Сейчас нет подтверждённо недоступных ресурсов в этом контуре.":"Для ресурсов нет свежих измерений.");return}
  el.innerHTML=rows.map(function(row){
    var pts=(row.points||[]).slice().sort(function(a,b){return Number(a.timestamp)-Number(b.timestamp)}),resource=resources.find(function(r){return Number(r.id)===Number(row.id)})||{},latest=latestPoint(row),rowNow=Date.now()/1000,isStale=!!(latest&&rowNow-Number(latest.timestamp)>staleAfter),state=latest&&!isStale?timelineState(latest):"unknown";
    ...
    var currentState=state==="down"&&scope==="RUSSIA"&&isReachable((resource.external||{}).status)?"Вероятное ограничение в РФ":timelineCurrentLabel(latest,state,isStale);
    var reason=timelineReason(latest,state,isStale,staleAfter);
    ...
    await withBusy(button,"Добавляем…",async function(){
      ...
      if(customTarget&&!S.customCatalogMatch){
        ...
        var custom=await api("/api/resources",{method:"POST",body:JSON.stringify({
          ...
          interval_seconds:60,expected_status_min:200,expected_status_max:399,slow_threshold_ms:1500,failure_threshold:2,enabled:true,alerts_enabled:(q("#customResourceGroup").value||"CUSTOM")==="CUSTOM"
        })});
...
function setup(){
  ...
  qa("[data-timeline-filter]").forEach(function(b){b.onclick=function(){var scope=b.closest(".timeline-filter").dataset.scope;S.timelineFilters[scope]=b.dataset.timelineFilter;if(scope==="GLOBAL")renderRealtime();else renderDomesticRealtime()}});
