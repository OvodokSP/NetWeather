...
- Российский контур (`RUSSIA`) измеряется отдельно backend через публичные Globalping-точки с выбором `country=RU`; Android (`USER`) в этот контур не попадает.
...
Globalping запускается только для нового сбоя или ручной диагностики. Использование квоты хранится в SQLite, 30% резерва защищено для ручных и новых критических проверок, результаты опрашиваются и классифицируются. OONI/IODA вызываются в том же событийном контуре и кэшируются.
