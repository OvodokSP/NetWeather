    ...
    try:
        ...
        if classification == "DNS_FAILURE":
            ...
            if global_is_fresh and is_reachable(global_check["status"]) and valid >= 2 and independent_failures >= 2:
                status = "HTTP_ERROR"
        if result is None:
            probe_message = "Проверка РФ: Globalping не вернул результат измерения."
        elif result.status != "finished":
            provider_error = result.raw.get("error") if isinstance(result.raw, dict) else None
            probe_message = "Проверка РФ: Globalping завершил измерение со статусом " + result.status
            if provider_error:
                probe_message += " · " + str(provider_error)[:180]
        elif classification == "OK":
            probe_message = f"Проверка РФ: ответы получены с {summary.get('reachable', 0)} из {summary.get('total', 0)} точек Globalping."
        elif classification == "DNS_FAILURE":
            probe_message = f"Проверка РФ: DNS не разрешил адрес на {summary.get('resolver_failures', 0)} точках из {summary.get('valid', 0)}."
        elif classification == "UNKNOWN":
            valid = int(summary.get("valid") or 0)
            internal = int(summary.get("internal_failures") or 0)
            if valid == 0 or internal:
                probe_message = f"Проверка РФ: Globalping не получил пригодный ответ ({internal} внутренних сбоев точек из {summary.get('total', 0)}). Доступность ресурса этим измерением не подтверждена."
            else:
                probe_message = f"Проверка РФ: неоднозначный результат — ответили {summary.get('reachable', 0)} из {valid} точек; ошибки на {summary.get('failed', 0)}."
        else:
            probe_message = f"Проверка РФ: {classification}; успешных ответов {summary.get('reachable', 0)} из {summary.get('valid', 0)}, ошибок {summary.get('failed', 0)}."
...
def _insert_catalog_resource(conn, item, now:int, *, interval_seconds:int=DEFAULT_INTERVAL,
                             ...
                             alerts_enabled:bool=False, enabled:bool=True) -> tuple[int,bool]:
...
def realtime_combined(minutes:int=Query(default=60,ge=5,le=10080)):
    ...
    with db() as conn:
        ...
        rows=conn.execute("""SELECT resource_id,probe_scope,checked_at,status,response_time_ms,dns_ms,tcp_ms,tls_ms,http_ms,http_status,message
    ...
    for row in rows:
        ...
        points.append({"timestamp":int(row["checked_at"]),"state":state,"status":row["status"],
                       "latency_ms":row["response_time_ms"],"http_status":row["http_status"],
                       "message":row["message"],"checks":1})
    result={}
    for scope in scopes:
        scope_resources=[]
        for resource in resources:
